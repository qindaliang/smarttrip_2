...
public class ParkingActivity extends AppCompatActivity implements OnGetPoiSearchResultListener, View.OnClickListener, OnRefreshLoadMoreListener {
    ...
    public void onViewClicked(View view) {
        ...
        switch (view.getId()) {
            ...
            case R.id.tv_parking_distance:
                setPoiNearbySearchOption(PoiSortType.distance_from_near_to_far, 1, 2000);
                rlvParking.setAdapter(mAdapter);
...
//                        .poiUid(poiInfo.uid));
            ...
            mAdapter.setOnItemClickListener(new ParkingRecyclerViewAdapter.OnItemClickListener() {
                ...
                public void onItemClick(View view, ParkingRecyclerViewAdapter.ViewName viewName, int postion) {
                    if (ParkingRecyclerViewAdapter.ViewName.ITEM == viewName) {
                        ...
                        String name = mList.get(postion).getName();
                        Intent intent = new Intent();
                        intent.putExtra(ConstantValues.PARKING_POI, name);
                        intent.putExtra(ConstantValues.PARKING_LAT, mList.get(postion).getLat());
                        intent.putExtra(ConstantValues.PARKING_LON, mList.get(postion).getLon());
                    ...
                    if (ParkingRecyclerViewAdapter.ViewName.IMAGEVIEW == viewName) {
                        ToastUtils.showBgResource(ParkingActivity.this, "跳转" + postion);
                    }
                }
            });
        }
    ...
    public void onGetPoiDetailResult(PoiDetailResult result) {
        if (result.error != SearchResult.ERRORNO.NO_ERROR) {
            ToastUtils.showBgResource(ParkingActivity.this, "抱歉，未找到结果");
        } else {
            StringBuilder sb = new StringBuilder(256);
            sb.append("\ngetAddress-------");
    ...
    public void onLoadMore(RefreshLayout refreshLayout) {
        ...
        if (firstPage < mTotalPageNum) {
            ...
            refreshParking.finishLoadMoreWithNoMoreData();
        }
    }
