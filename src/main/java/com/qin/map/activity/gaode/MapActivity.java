...
import com.qin.R;
import com.qin.map.aoverlay.DrivingRouteOverlay;
import com.qin.map.aoverlay.WalkRouteOverlay;
import com.qin.map.route.BusResultListAdapter;
import com.qin.map.route.DriveRouteDetailActivity;
import com.qin.map.route.WalkRouteDetailActivity;
import com.qin.map.util.AMapUtil;
import com.qin.util.ToastUtils;

/**
 * Route路径规划: 驾车规划、公交规划、步行规划
 */
public class MapActivity extends Activity implements OnMapClickListener, OnMarkerClickListener, OnInfoWindowClickListener, InfoWindowAdapter, OnRouteSearchListener {
    private AMap aMap;
...
//		getIntentData();
    ...
    private void init() {
        ...
        findViewById(R.id.btn_map_type).setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                searchRouteResult(ROUTE_TYPE_DRIVE, RouteSearch.DRIVING_MULTI_CHOICE_AVOID_CONGESTION_NO_HIGHWAY_SAVE_MONEY);
            }
        });
    }
...
//    }
    ...
    public void searchRouteResult(int routeType, int mode) {
        if (mStartPoint == null) {
            ToastUtils.showBgResource(mContext, "起点未设置");
            return;
        }
        if (mEndPoint == null) {
            ToastUtils.showBgResource(mContext, "终点未设置");
        }
        showProgressDialog();
        ...
        if (errorCode == AMapException.CODE_AMAP_SUCCESS) {
            if (result != null && result.getPaths() != null) {
                if (result.getPaths().size() > 0) {
                    ...
                    ToastUtils.showBgResource(mContext, "对不起，没有搜索到相关数据！");
                }
            } else {
                ToastUtils.showBgResource(mContext, "对不起，没有搜索到相关数据！");
            }
        } else {
            ToastUtils.showBgResource(this.getApplicationContext(), String.valueOf(errorCode));
        }
    }
